...
async def get_stats(db: AsyncSession = Depends(get_db)):
    ...
    # Total feeds
    result = await db.execute(select(func.count(Feed.id)))
    ...
    result = await db.execute(
        select(func.count(Paper.id)).where(Paper.fetched_at >= today_start)
    )
    today_papers = result.scalar() or 0

    # Today's analyses
    result = await db.execute(
        select(func.count(AnalysisResult.id)).where(AnalysisResult.analyzed_at >= today_start)
    )
    ...
    # Total keywords
    result = await db.execute(select(func.count(Keyword.id)))

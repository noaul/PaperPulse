    ...
    <div class="grid grid-cols-1 xl:grid-cols-3 gap-6">
      <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-5">
        <div class="flex items-center justify-between mb-4">
          ...
          <span class="text-sm text-gray-500">{{ reports.length }} 份</span>
        </div>

        <div v-if="reportsLoading" class="flex justify-center py-10">
          <div class="animate-spin rounded-full h-8 w-8 border-b-2 border-blue-600"></div>
        </div>

        <div v-else-if="reports.length === 0" class="text-sm text-gray-500 text-center py-10">
          暂无报告，先生成一份今日报告。
        </div>

        <div v-else class="space-y-3 max-h-[calc(100vh-260px)] overflow-y-auto pr-1">
          <button
          ...
          >
            ...
            <div class="grid grid-cols-3 gap-2 mt-3 text-xs">
              <div class="rounded bg-white/80 p-2">
                <span class="block text-gray-500">论文</span>
                <strong class="text-gray-900">{{ report.paper_count }}</strong>
              </div>
              <div class="rounded bg-white/80 p-2">
                <span class="block text-gray-500">阈值</span>
                <strong class="text-gray-900">{{ report.threshold.toFixed(1) }}</strong>
              </div>
              <div class="rounded bg-white/80 p-2">
                <span class="block text-gray-500">最高分</span>
                <strong class="text-gray-900">{{ report.max_relevance_score.toFixed(1) }}</strong>
              </div>
            </div>
          </button>
        </div>
      ...
      <div class="xl:col-span-2 bg-white rounded-xl shadow-sm border border-gray-200 p-5 min-h-[520px]">
        ...
        <div v-else-if="!selectedReport" class="text-sm text-gray-500 text-center py-16">
